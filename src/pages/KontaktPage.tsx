      ...
      <section className="relative py-20 blue-gradient overflow-hidden">
        <div className="container-narrow px-4">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="font-heading text-4xl md:text-6xl text-primary-foreground mb-4">Jetzt Schaden melden!</h1>
            <p className="text-primary-foreground/70 text-lg max-w-2xl">Kostenlose Beratung für Ihren Schadensfall in Braunschweig und Umgebung.</p>
          </motion.div>
        </div>
      </section>
                ...
                {submitted ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-foreground mb-2">Telefon *</label>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-foreground mb-2">Nachricht *</label>
                    ...
                    <motion.button type="submit" className="w-full gold-gradient text-accent-foreground font-bold py-4 text-base tracking-wide rounded-lg flex items-center justify-center gap-2 shadow-lg shadow-accent/20" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                      <Send className="h-5 w-5" /> Jetzt Schaden melden
                    </motion.button>
                  </form>
                )}
              </div>
            ...
            <ScrollReveal direction="right" delay={0.2}>
              <div className="space-y-6">
                <div className="bg-card border border-border rounded-2xl p-8">
                  <h3 className="font-heading text-2xl text-foreground mb-6">ING Kfz Gutachten</h3>
                  <div className="space-y-5">
                    <a href="tel:053122436430" className="flex items-center gap-4 hover:text-primary transition-colors">
                      ...
                      <div><span className="font-bold block">0531 - 22 436 430</span><span className="text-muted-foreground text-xs">Jetzt anrufen</span></div>
                    </a>
                    <a href="mailto:info@kfz-sachverstaendiger-braunschweig.de" className="flex items-center gap-4 hover:text-primary transition-colors">
                      ...
                      <div><span className="block text-sm">info@kfz-sachverstaendiger-braunschweig.de</span><span className="text-muted-foreground text-xs">E-Mail</span></div>
                    </a>
                    <div className="flex items-center gap-4">
                      ...
                      <div><span className="block font-medium">Braunschweig & Region</span><span className="text-muted-foreground text-xs">Vor-Ort-Service</span></div>
                    </div>
                    <div className="flex items-center gap-4">
                      ...
                      <div><p className="font-medium">Mo – Fr: 08:00 – 18:00</p><p className="text-muted-foreground text-xs">Sa: nach Vereinbarung</p></div>
                    </div>
                  </div>
                </div>
                <div className="border-2 border-primary/20 bg-primary/5 p-6 rounded-2xl">
                  <h4 className="font-heading text-xl text-foreground mb-2 flex items-center gap-2">
                    <ArrowRight className="h-5 w-5 text-primary" /> Im Haftpflichtfall kostenfrei
                  </h4>
                  <p className="text-muted-foreground text-sm leading-relaxed">Die gegnerische Versicherung übernimmt die Kosten für das Gutachten.</p>
                </div>
              </div>
            </ScrollReveal>
          </div>
